...
import type { Tag, NewTag } from "~/db";
...
export const createPostWithTags = createServerFn({ method: "POST" })
	.validator((d: { title: string; body?: string; status?: string; tagNames: string[] }) => d)
	.handler(async ({ data }) => {
		const supabase = getSupabaseServerClient();
...
export const updatePostWithTags = createServerFn({ method: "POST" })
	.validator((d: { id: string; title?: string; body?: string; status?: string; tagNames: string[] }) => d)
	.handler(async ({ data }) => {
		const supabase = getSupabaseServerClient();
