...
function PostComponent() {
	...
	return (
		<PostForm
			...
			error={updateMutation.status === "error" ? `Failed to save post: ${updateMutation.error?.message}` : undefined}
			showDeleteButton={true}
			onDelete={handleDelete}
			isDeletingButtonText="Delete Post"
			isDeleting={deleteMutation.status === "pending"}
			deleteError={deleteMutation.status === "error" ? `Failed to delete post: ${deleteMutation.error?.message}` : undefined}
		/>
	);
}
